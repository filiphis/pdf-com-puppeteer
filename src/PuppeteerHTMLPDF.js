...
class PuppeteerHTMLPDF {
  ...
  async setOptions(options) {
    ...
    if (has(options, 'browserWSEndpoint')) {
      this.browser = await puppeteer.connect({ browserWSEndpoint: options.browserWSEndpoint });
    } else {
  ...
  async initializeBrowser() {
    ...
    this.browser = await puppeteer.launch({ args: this.args, headless });
...
export default PuppeteerHTMLPDF;